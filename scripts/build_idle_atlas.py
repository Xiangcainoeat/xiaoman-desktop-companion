#!/usr/bin/env python3
"""Build the host-only transparent idle-action atlas from a generated sheet."""
...
def detect_regions(source: Image.Image) -> list[tuple[int, int, int, int]]:
    ...
    row_runs = contiguous_runs(foreground.sum(axis=1) > 20, merge_gap=3)
    ...
    for top, bottom in row_runs:
        column_runs = contiguous_runs(foreground[top:bottom].sum(axis=0) > 3, merge_gap=5)
        if len(column_runs) != COLUMNS:
            raise ValueError(
                f"expected {COLUMNS} frames in row {top}:{bottom}, found {len(column_runs)}: {column_runs}"
            )
...
def extract_frame(source: Image.Image, region: tuple[int, int, int, int], index: int) -> Image.Image:
    left, top, right, bottom = region
    keyed = chroma_to_alpha(source.crop((left, top, right, bottom)))
    bbox = keyed.getchannel("A").point(lambda value: 255 if value >= 10 else 0).getbbox()
    if bbox is None:
    ...
    target_size = (max(1, round(foreground.width * scale)), max(1, round(foreground.height * scale)))
...
def make_contact_sheet(atlas: Image.Image) -> Image.Image:
    ...
    for index in range(FRAME_COUNT):
        ...
        label = f"{ACTIONS[index // COLUMNS]} {index % COLUMNS + 1}"
        draw.rectangle((x + 3, y + 3, x + 74, y + 16), fill=(255, 255, 255, 224))
