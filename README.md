...
- 睡觉使用完整 30 帧蜷睡图集；增强模式的喂食复用原生色舔嘴图集，洗澡复用原生站立循环并只做微小离散播放位移，不做透明叠加
- 喂养、摸摸、玩耍、睡觉、叫醒和庆祝互动
...
- 悬停工具栏中的心形：在与 Codex 任务相同的位置和尺寸打开“养成照料”面板；手柄：切换到同一宿主里的“互动”面板
- 养成、互动和 Codex 任务始终只显示一个面板；按住小满本体拖动时，面板和小满一起移动，标题区域不会拖动窗口
- “互动”面板中的“更多游戏”：直接打开控制中心的“互动游戏”视图
- 点击 `</>`：在悬浮窗中选择任务并直接回复
- 菜单栏图标：显示/隐藏、互动或退出
...
干净发布仓库还包含 `codex-pet/`：其中既有可直接安装的原生 Codex 两文件宠物，也有可由 Codex 复用的 `hatch-pet` 技能、确定性脚本、测试、QA 证据和小满完整制作案例。`public/pet/native/` 是宿主内置的原生 profile 副本，`work/xiaoman-pet-96/` 保留本机 CLI 生图提示、选中结果、扩帧 provenance、图集验证和 QA 材料，`work/xiaoman-care-assets/` 保留睡眠、护理和小游戏素材的来源与验证记录；本次发布的生图与 Agent 并发硬上限为 4，实际生图串行完成。宿主不会写回 `~/.codex/pets/xiaoman`。
