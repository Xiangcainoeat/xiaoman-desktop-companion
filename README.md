...
小满桌面伴侣 1.3.0 是一个独立的 macOS 应用，为小满增加透明桌面悬浮窗、平滑注视、互动养成、提醒、声音、Codex 任务控制和外部应用事件。
...
- 增强模式的 96 方向注视使用独立头部图层：身体锁定在注视开始时的动作帧，只更新头部，低头阶段平滑收敛且不做时间透明混合
...
控制中心包含“概览”“功能管理”“Codex 任务”“提醒计划”“应用事件”“设置”六个视图。所有扩展能力均可在“功能管理”中独立开关；注视模式、刷新率、鼠标静止多久停止跟随、悬停跳跃次数和体型在“设置”中调整。
...
干净发布仓库还包含 `codex-pet/`：其中既有可直接安装的原生 Codex 两文件宠物，也有可由 Codex 复用的 `hatch-pet` 技能、确定性脚本、测试、QA 证据和小满完整制作案例。`public/pet/native/` 是宿主内置的原生 profile 副本，`work/xiaoman-pet-96/` 保留本机 CLI 生图提示、选中结果、扩帧 provenance、图集验证和 QA 材料。宿主不会写回 `~/.codex/pets/xiaoman`。
