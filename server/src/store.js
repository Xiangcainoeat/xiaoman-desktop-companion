...
export class SocialStore {
  ...
  rematch(roomId, userId, now) {
    this.cleanupExpiredRooms(now);
    this.assertRoomPlayer(roomId, userId);
    const row = this.db.prepare("SELECT game_id FROM rooms WHERE id = ?").get(roomId) ?? null;
    if (!row) throw new AppError("ROOM_NOT_FOUND", "找不到这个游戏房间", 404);
    const room = this.roomById(roomId);
    if (room.status === "left") throw new AppError("INVALID_ROOM_STATE", "这个房间已结束", 409);
    this.transaction(() => {
      this.db.prepare("UPDATE rooms SET status = ?, turn = 'red', seq = 0, position = ?, last_move_json = NULL, winner = NULL, updated_at = ? WHERE id = ?").run(this.roomPlayerRows(roomId).length === 2 ? "ready" : "waiting", initialPosition(row.game_id), now, roomId);
