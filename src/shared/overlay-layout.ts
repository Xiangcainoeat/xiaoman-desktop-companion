...
export function overlayDimensions(petSize: number, taskPanelOpen: boolean): { width: number; height: number } {
  ...
  return {
    width: petWidth + (taskPanelOpen ? QUICK_TASK_PANEL_WIDTH : 0),
    height: taskPanelOpen ? Math.max(petHeight, QUICK_TASK_PANEL_MIN_HEIGHT) : petHeight,
  };
}
