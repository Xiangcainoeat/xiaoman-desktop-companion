...
function isRoomActionRequest(value: unknown): boolean {
  return isRecord(value)
    && typeof value.requestedByUserId === "string"
    && value.requestedByUserId.length > 0
...
function isRoom(value: unknown): value is GameRoom {
  ...
  return (value.status === "waiting" || value.status === "ready" || value.status === "playing"
    ...
    && (value.undoRequest === undefined || value.undoRequest === null || isRoomActionRequest(value.undoRequest))
    && (value.rematchRequest === undefined || value.rematchRequest === null || isRoomActionRequest(value.rematchRequest))
    && typeof value.createdAt === "number"
    && typeof value.updatedAt === "number"
