...
export class GuestLocalTransport implements SocialTransport {
  ...
  async resign(roomId: string): Promise<void> {
    ...
    if (room.status !== "playing") {
      throw new SocialError("INVALID_ROOM_STATE", "当前房间不能认输");
    }
  ...
  async rematch(roomId: string): Promise<void> {
    ...
    if (room.status !== "finished") {
      throw new SocialError("INVALID_ROOM_STATE", "只有已结束的对局可以再来一局");
    }
    if (!room.players.red || !room.players.black) {
      throw new SocialError("INVALID_ROOM_STATE", "需要双方都在房间内才能再来一局");
    }

    const request = room.rematchRequest;
    if (!request) {
      room.rematchRequest = { requestedByUserId: user.id, requestedAt: this.now() };
      room.undoRequest = null;
      room.players.red.ready = room.players.red.user.id === user.id;
      room.players.black.ready = room.players.black.user.id === user.id;
      this.touchRoom(room);
      this.emit({ type: "room.updated", room });
      return;
    }
    if (request.requestedByUserId === user.id) return;
    ...
    room.rematchRequest = null;
    this.roomHistory.delete(room.id);
    room.players.red.ready = true;
    room.players.black.ready = true;
    this.touchRoom(room);
    this.emit({ type: "room.updated", room });
  }
