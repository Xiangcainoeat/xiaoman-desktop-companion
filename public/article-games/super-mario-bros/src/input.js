(function() {
  ...
  // Supports keyboard, and a drawn touchpad activated only on touch/mobile surfaces.
  Backbone.Input = Backbone.Model.extend({
    defaults: {
      ...
      drawKeyboardHint: false, // Desktop host uses the surrounding Chinese keymap instead.
    ...
    hasTouchpad: function() {
      ...
      if (_.isBoolean(drawTouchpad)) return drawTouchpad === true && this.isTouchOrMobileSignal();
      if (drawTouchpad == "auto") return this.isTouchOrMobileSignal();
      return false;
    },
    isTouchOrMobileSignal: function() {
      var coarsePointer = window.matchMedia && window.matchMedia("(pointer: coarse)").matches;
      var touchPoints = Number(window.navigator.maxTouchPoints || window.navigator.msMaxTouchPoints || 0);
      return Boolean(this.get("touchEnabled") && (coarsePointer || touchPoints > 0 || "ontouchstart" in window));
    },
    ...
    draw: function(context, options) {
      ...
      } else if (this.get("drawKeyboardHint")) {
