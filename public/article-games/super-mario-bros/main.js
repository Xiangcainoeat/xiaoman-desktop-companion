$(window).on("load", function() {
  ...
  Backbone.Controller = Backbone.Model.extend({
    initialize: function(attributes, options) {
      ...
      // Local frame adaptation: only mobile/touch surfaces draw the upstream touchpad.
      ...
      this.world = new Backbone.World(
        _.extend({viewportBottom: this.input.isTouchOrMobileSignal() ? 156 : 0}, window._world), {
        input: this.input,
        camera: this.camera
      });
